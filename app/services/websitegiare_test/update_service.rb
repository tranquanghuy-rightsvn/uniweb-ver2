class WebsitegiareTest::UpdateService
  ...
  def perform
    ...
    stt = WebsitegiareTest.where("id > ?", resource.id).count
    if stt > 19
      doc_store = Nokogiri::HTML(File.read('projects/websitegiare_test/media-luu-tru.html'))
      div_store = doc_store.at_css('#render-store')
      new_element = Nokogiri::HTML("<div class=\"col-lg-6 col-md-12\">
                                <div class=\"box-store\">
                                  <div class=\"store-header\">
                                    <p> Admin</p>
                                    <i>" + resource.time_publish.to_s + "</i>
                                  </div>
      ...
      div_store.reject{|element| element.blank?}[stt - 19].replace(new_element)
      ...
      div_media.children.reject{|element| element.blank?}[stt].replace(new_element)
    end
