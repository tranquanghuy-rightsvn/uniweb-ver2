...
# websitegiare-daa3c70f8d92.json
  def perform
    content = Google::Apis::AnalyticsdataV1alpha::AnalyticsDataService.new

    scope = 'https://www.googleapis.com/auth/content'

    content.authorization = Google::Auth::ServiceAccountCredentials.make_creds(
      json_key_io: File.open(Rails.root.join('public', 'websitegiare-daa3c70f8d92.json')),
      scope: scope)

    content.authorization.fetch_access_token!
    a = content.run_property_realtime_report('410439060')
    byebug
  end
